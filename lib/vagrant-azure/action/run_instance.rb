    ...
    module Action
      class RunInstance
        ...
        def call(env)
          ...
          # use a storage account name that is the VM name hashed within the limits
          # of the storage account names provided by azure
          storage_account_name           = Digest::MD5.hexdigest(vm_name)[0..20]

          # enable us to define security configuration with audit details for
          # our machine/compute externally and load them into the ARM for full
          # control of the security groups. This functions the same as `tcp_endpoints`
          # except that we want to provide a JSON file containing the security group ARM
          ...
          env[:ui].info(" -- Storage Type: #{vm_storage_account_type}") if vm_storage_account_type
          ...
          deployment_params = {
            ...
            addressPrefix:        vm_network_prefix,
          ...
          template_params.merge!(security_config: get_security_config(security_config))
        ...
        def get_security_config(file)
          security_config = []
          unless file.nil?
            security_config = JSON.parse(File.open(file, "rb").read)
          end
          security_config
        end
